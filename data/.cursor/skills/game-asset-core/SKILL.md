...
description: >
  Core discipline for game-asset generation: engine-ready defaults, spec
  checklists, style anchoring, read-back verification, honest defect flagging.
  Image tools: Higgsfield or Grok imagine_* only if listed — never invent them;
  otherwise CSS/SVG/canvas. Use whenever generating game art (sprites, sheets,
  animations, tiles, UI, FX) — then ALSO load the matching specialist:
  game-animation-frames, game-tilesets, game-character-consistency, game-ui-icons.
metadata:
...
This skill is **doctrine / QC**. For 2D sprites and motion, also run the pipeline skills:
...
**Image tools:** read [references/image-tools.md](references/image-tools.md) first.
Never invent `imagine_*` calls. Prefer Higgsfield `generate_image` / `generate_video` when listed; otherwise CSS/SVG/canvas.
