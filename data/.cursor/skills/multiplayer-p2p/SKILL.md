...
ping), and `candidateType` (`host`/`srflx` = direct). To override STUN, set
`VITE_STUN_URLS` (comma-separated) in the game app's existing env files and
restart that app's dev server. Never create a new `.env` for this; never write
Grok `.grok/app-env.json`.
