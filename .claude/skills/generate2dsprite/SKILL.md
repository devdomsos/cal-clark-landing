...
metadata:
  short-description: "2D sprite sheets: available image tool + magenta chroma postprocess"
user-invocable: false
---

# Generate2dsprite

Use this skill for self-contained 2D sprite or animation assets in **browser
games** in this monorepo (for example Dance or Die). Not a TanStack Start sandbox.
...
code-drawn placeholders **when an image tool is actually available**.
If no image tool is listed, geometric/CSS/canvas art is the correct path
([image-tools.md](../game-asset-core/references/image-tools.md)).

## Environment (this repo)

| Item | Value |
| --- | --- |
| Skill dir | `.cursor/skills/generate2dsprite/` |
| Scripts | `python3 .cursor/skills/generate2dsprite/scripts/<script>.py …` |
| Image tools | See **`game-asset-core/references/image-tools.md`**. Use Higgsfield `generate_image` when listed. Never invent `imagine_*`. |
| Inspect images | Read the PNG path with the file/image tool |
| Generated image path | Copy the tool's output into the game's `assets/sprites/<name>/` before processing |
| Output home | The consuming game's `assets/sprites/<name>/` |
| Engine target | Browser: Canvas 2D, Phaser, or DOM/`<img>` |

Related skills: **`game-asset-core`**, **`generate2dmap`**, **`video2dsprite`**, **`building-games`**.

Where later sections say `imagine_text_to_image` / `imagine_image_to_image`, substitute the image tool that is actually in your tool list (usually Higgsfield `generate_image`). Never invent `imagine_*`.
...
- Generate raw sprite art with the **available** image tool (`imagine_*` only if listed; otherwise Higgsfield `generate_image`). Never invent `imagine_*`. If no image tool exists, ship CSS/SVG/canvas art and skip this pipeline.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots **when an image tool is listed**. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images.
- When the user provides a visual reference, pass that file into the image-edit tool your list actually has. Also read the local reference so you can see it; a path mentioned only inside the prompt is not a visual input.
...
```bash
python3 .cursor/skills/generate2dsprite/scripts/make_layout_guide.py \
  --rows <rows> \
  --cols <cols> \
  --cell-width 384 \
  --cell-height 384 \
