...
name: building-games
description: >
  Build browser games and interactive/canvas/3D experiences in this monorepo
  (React web or a game package such as Dance or Die). Use for any game,
  simulation, or WebGL/Canvas experience — 2D or 3D, single-player. Covers the
  game loop & timing, 3D orientation/camera conventions, collision, performance,
  assets, audio, save, game feel, and per-genre playbooks. For WASD / vehicle /
  flight **input signs and inverted A/D**, open the **`controls`** skill — do
  not rely on this file or racing-kart alone. Triggers on "game", "minecraft",
  "fps", "platformer", "racing", "tetris", "snake", "shooter", "3d", "three.js",
  "canvas", "voxel", "physics".
metadata:
...
**Controls / inverted A/D:** open **`.cursor/skills/controls/SKILL.md`** **before**
...
**Open `.cursor/skills/controls/SKILL.md` before implementing any WASD / steer /
...
- Reserve image generation for **flat 2D** assets only (textures, sky/menu
  ...
  See [image-tools.md](../game-asset-core/references/image-tools.md). Never invent `imagine_*`.
  Abstract games (tetris, snake, pong, breakout) stay procedural even when gen tools exist.
- **Engine-ready game art doctrine** → open **`game-asset-core`**
  ...
  **`game-tilesets`**, **`game-character-consistency`**, **`game-ui-icons`**.
  ...
  placeholders when real art is expected **and an image tool is listed**.
- **2D game sprites / animation sheets** → run **`generate2dsprite`**
  (`.cursor/skills/generate2dsprite/SKILL.md`): solid **`#FF00FF`** magenta
  sheets + chroma postprocess scripts when generating raster art.
- **2D maps / levels / prop packs** → open **`generate2dmap`**
  (`.cursor/skills/generate2dmap/SKILL.md`). Prefer foundation-only base + separate
...
- **Optional denser locomotion** → run **`video2dsprite`** when an image→video
  tool exists (Higgsfield `generate_video`). Prefer `generate2dsprite`
  for crisp production heroes.
