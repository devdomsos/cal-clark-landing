...
name: generate2dmap
description: >
  Generate production-oriented 2D game maps: RPG/top-down maps, side-scroller
  parallax stages, tilemaps, layered raster maps, prop packs, collision zones,
  and walkable areas. Use when building browser games that need real map art
  (not pure code-drawn tiles), layered props, or map collision metadata.
  Image tools: see game-asset-core/references/image-tools.md — never invent imagine_*.
  Triggers on "map", "level", "stage", "tilemap", "overworld", "dungeon",
  "side scroller background", "prop pack", "2D map".
metadata:
...
| Item | Value |
| --- | --- |
| Skill dir / scripts | `.cursor/skills/generate2dmap/`, run as `python3 .cursor/skills/generate2dmap/scripts/<script>.py …` |
| Image tools | See **`game-asset-core/references/image-tools.md`**. Never invent `imagine_*`. |
| Generated image path | Copy tool output into the game's `assets/map/` |
| Default `engine_target` | `raw_canvas` or Phaser unless the user wants Tiled/Godot/Unity |
| Related skills | **`generate2dsprite`**, **`building-games`**, **`game-asset-core`** |
...
- Image generation is the default art source for base maps, parallax plates,
  references, prop sheets, and tileset art **when a tool is listed** (Higgsfield
  `generate_image`, or `imagine_*` on Grok Build). Default `art_style` is `clean_hd`
  ...
  No image tool → procedural/CSS/canvas tiles are the correct path.
